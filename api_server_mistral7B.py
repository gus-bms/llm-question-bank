...
# 모델 설정
MODEL_PATH = "/mnt/storage/models/mistral-7B-Instruct-v0.2"
        ...
        with Timer("모델 로드"):
            ...
            try:
                tokenizer = AutoTokenizer.from_pretrained(
                    ...
                    use_fast=True,  # Llama 토크나이저는 fast 버전 사용
                    padding_side="left",
            ...
            try:
                model = AutoModelForCausalLM.from_pretrained(
                    MODEL_PATH,
                    torch_dtype=TORCH_DTYPE,  # FP16 사용
                    device_map="auto",
...
async def startup_event():
    ...
    try:
        ...
        # 모델 로드
        logger.info("Mistral 모델을 로드합니다...")
        model, tokenizer = load_model()
        if model is None or tokenizer is None:
            raise Exception("모델 또는 토크나이저 로드 실패")
            
        logger.info("Mistral 모델이 성공적으로 로드되었습니다.")
